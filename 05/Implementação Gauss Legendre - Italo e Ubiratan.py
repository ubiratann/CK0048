...
def aproximador(f,x,a,b,quantpontos,numeroDivisoes):
...
def calcular(f,x,qntPontos,lInferior=0, lSuperior=1 ):
    erro = 10**(-6)

    valorAnterior = aproximador(f,x,lInferior, lSuperior,qntPontos,0)
    valorAtual = aproximador(f,x,lInferior, lSuperior,qntPontos,1)
    iteracoes = 1
    while(abs(valorAnterior-valorAtual)>erro):
        iteracoes +=1
        valorAnterior = valorAtual
        valorAtual = aproximador(f,x,lInferior, lSuperior,qntPontos,iteracoes)
